...
| 等級 | 分數 | 意義 |
|------|------|------|
| **A** | 75-100 | 高品質 VCP / 強勢趨勢，優先研究 |
| **B** | 60-74  | 良好設置，適合加入重點觀察 |
| **C** | 45-59  | 早期成形，可作觀察清單 |
| **D** | 0-44   | 條件較弱，只作備選或略過 |
...
收到的 Email 包含：
- 每隻股票的 **評分（0-100）與 A/B/C/D 等級**
- 現價、**入場位 / 目標價 / 停損位**（技術面交易計畫）
- Stage 階段、收縮次數、RS 評分
- MM 趨勢模板 9 項通過狀況（✅/❌）
- 華爾街式分析師評級（`Strong Buy` / `Buy` / `Outperform Watchlist` / `Neutral`）、信心、上行/下行空間、RSI、風險報酬比（R 倍數）
...
```yaml
MIN_SCORE: "40"          # 總分門檻（建議 35~50；預設已放寬）
TREND_MIN_PASSED: "6"    # MM 趨勢模板最少通過幾項（0~9，建議 5~7）
MIN_CONTRACTIONS: "0"    # 最少收縮次數（建議 0~1，避免早期整理被剔除）
MIN_RESULTS: "10"        # 若達標數不足，自動補入最接近門檻候選作觀察名單
ONLY_BUY_RECOMMENDATION: "false" # 是否只輸出 BUY（建議先 false，避免清單過少）
```

> 若你仍然「完全沒有股票」，先把 `MIN_SCORE` 降到 35，或把 `TREND_MIN_PASSED` 降到 5；`MIN_RESULTS` 會在達標數不足時補入最接近門檻的候選股。
